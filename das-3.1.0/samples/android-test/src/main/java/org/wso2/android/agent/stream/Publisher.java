...
public class Publisher{
    ...
    private static final String[] DEVICES = {"0000000000","1111111111","2222222222","3333333333","4444444444",
    ...
    private static void publishEvent(DataPublisher dataPublisher, String streamId) throws FileNotFoundException, SocketException, UnknownHostException {
        String content = String.format("{\"name\":\"TestEvent\",\"state\":true,\"time\":%d}", System.currentTimeMillis());
        Object[] payload = new Object[]{
                DEVICES[RAND.nextInt(10)],
                content,
                "TEST_STATE"};
        Event event = new Event(streamId, System.currentTimeMillis(), null, null, payload);
        dataPublisher.publish(event);
        System.out.println("Published 1 event - " + content);
    }
    private static void publishEvents(DataPublisher dataPublisher, String streamId) throws FileNotFoundException, SocketException, UnknownHostException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Object[] payload = new Object[]{
                    DEVICES[RAND.nextInt(10)],
                    String.format("{\"name\":\"TestEvent\",\"state\":true,\"time\":%d}", System.currentTimeMillis()),
                    "TEST_STATE"
            };
            Event event = new Event(streamId, System.currentTimeMillis(), null, null, payload);
            dataPublisher.publish(event);
